...
export async function readDataRawRows() {
  ...
  const res = await sheets.spreadsheets.values.get({
    ...
    // UNFORMATTED_VALUE so a date cell comes back as its raw serial number
    // (locale-proof) instead of a display string we'd have to re-parse.
  ...
  return rows
    .map((r, i) => {
      ...
      const serial = r[2];
      let enquiry_date = null;
      if (typeof serial === "number" && serial > 0) {
        // Excel/Sheets day-0 is 1899-12-30.
        const ms = Math.round((serial - 25569) * 86400 * 1000);
        const d = new Date(ms);
        if (!isNaN(d.getTime())) enquiry_date = d.toISOString().slice(0, 10);
      }
