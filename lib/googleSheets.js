...
function getAuth() {
  ...
  // The key reaches us differently depending on where it was pasted:
  // .env.local strips its surrounding quotes, but Vercel's settings page
  // keeps them if they were pasted in, and may store literal "\n" text or
  // Windows line endings. Any of those makes the key unreadable
  // ("DECODER routines::unsupported"), so normalize all of them.
  const key = rawKey
    .trim()
    .replace(/^["']+|["']+$/g, "")
    .replace(/\\r/g, "")
